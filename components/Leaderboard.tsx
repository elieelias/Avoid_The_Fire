...
import { Medal, Trophy, ChevronLeft, Home } from 'lucide-react';
...
const Leaderboard: React.FC<Props> = ({ player, setAppState: _setAppState, setPlayer }) => {
        ...
        try {
            ...
            let entries: LeaderboardEntry[] = rows
                ...
                .map((row) => ({
                    ...
                    score: row.score || 0,
                    isCurrentUser: player ? row.player_name === player.name && row.score === player.score : false,
                }))
                .sort((a, b) => b.score - a.score);

            const hasPlayer = player ? entries.some(
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 animate-fadeIn overflow-hidden">
            <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar relative">
                ...
                {/* Header (sits above leaderboard by default) */}
                <div className="bg-white p-6 pb-8 rounded-b-[2.5rem] shadow-sm text-center border-b border-slate-100 relative z-10 flex flex-col items-center">
                    <button
                        onClick={() => _setAppState(AppState.REGISTRATION)}
                        className="absolute top-6 left-6 text-slate-400 hover:text-slate-600 transition-colors flex items-center gap-1 font-semibold"
                    >
                        <ChevronLeft size={20} /> Back
                    </button>
                    <div className="w-16 h-16 bg-yellow-50 rounded-2xl flex items-center justify-center mb-3 mt-6">
                        <Trophy className="text-yellow-500" size={32} />
                    </div>
                    <h2 className="text-4xl font-black text-slate-800 mb-1 mt-6">Top Players</h2>
                    <div className="text-slate-500 mb-4">Can you beat the high score?</div>
                </div>
            ...
            <div className="bg-white p-6 border-t border-slate-100 z-10 flex flex-col gap-4">
                <Button
                    variant="outline"
                    fullWidth
                    onClick={() => _setAppState(AppState.REGISTRATION)}
